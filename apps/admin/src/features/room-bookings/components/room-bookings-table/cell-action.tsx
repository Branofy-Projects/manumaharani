...
import { updateRoomBookingStatus } from "@repo/actions";
import {
  ArrowRightCircle,
  Ban,
  CheckCircle,
...
type RoomBookingStatus = "cancelled" | "closed" | "confirmed" | "contacted" | "pending";

type TRoomBookingWithRoom = TRoomBooking & {
  room: { title: string } | null;
};

const STATUS_OPTIONS: { icon: React.ReactNode; label: string; value: RoomBookingStatus }[] = [
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onStatusChange = (status: RoomBookingStatus) => {
    ...
    startTransition(async () => {
      try {
        await updateRoomBookingStatus(data.id, status);
  ...
  const currentStatus = data.status as RoomBookingStatus;
      ...
      <Dialog onOpenChange={setDetailsOpen} open={detailsOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {data.room?.title || "Room booking"} request
            </DialogDescription>
          </DialogHeader>
