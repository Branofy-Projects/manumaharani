...
import { updateOfferBookingStatus } from "@repo/actions";
import {
  ArrowRightCircle,
  Ban,
  CheckCircle,
...
type OfferBookingStatus = "cancelled" | "closed" | "confirmed" | "contacted" | "pending";

type TOfferBookingWithOffer = TOfferBooking & {
  offer: { name: string } | null;
};

const STATUS_OPTIONS: { icon: React.ReactNode; label: string; value: OfferBookingStatus }[] = [
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onStatusChange = (status: OfferBookingStatus) => {
    ...
    startTransition(async () => {
      try {
        await updateOfferBookingStatus(data.id, status);
  ...
  const currentStatus = data.status as OfferBookingStatus;
      ...
      <Dialog onOpenChange={setDetailsOpen} open={detailsOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {data.offer?.name || "Offer booking"} request
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {new Date(data.travel_date).toLocaleDateString("en-US", {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })}
