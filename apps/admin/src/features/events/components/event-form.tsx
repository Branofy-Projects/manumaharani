...
const EventForm = (props: TEventFormProps) => {
  ...
  const [currentEventId, setCurrentEventId] = useState<string | undefined>(props.eventId);
  ...
  const ensureEventExists = async (): Promise<null | string> => {
    if (currentEventId) {
    ...
    try {
      const newEvent = await createEvent(minimalEventData);
      if (newEvent?.id) {
        setCurrentEventId(newEvent.id);
        toast.success("Event created! You can now save individual sections.");
        return newEvent.id;
      }
      return null;
    } catch (error) {
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          ...
                          <FormDescription>
                            Select the event date (or date range for multi-day events)
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            ...
                            <FormDescription>
                              Event free cancellation to customers
                            </FormDescription>
                          </div>
                          <FormControl>
                            <Switch
