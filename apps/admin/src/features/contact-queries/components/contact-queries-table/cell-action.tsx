...
import { updateContactQueryStatus } from "@repo/actions";
import {
  ArrowRightCircle,
  CheckCircle,
  Eye,
...
type ContactQueryStatus = "closed" | "contacted" | "pending" | "resolved";

const STATUS_OPTIONS: { icon: React.ReactNode; label: string; value: ContactQueryStatus }[] = [
  ...
  { icon: <CheckCircle className="mr-2 h-4 w-4" />, label: "Resolved", value: "resolved" },
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onStatusChange = (status: ContactQueryStatus) => {
    const statusLabel = STATUS_OPTIONS.find((s) => s.value === status)?.label || status;

    if (!confirm(`Change query status to "${statusLabel}"?`)) {
      return;
    }

    startTransition(async () => {
      try {
        await updateContactQueryStatus(data.id, status);
  ...
  const currentStatus = data.status as ContactQueryStatus;
