...
import { updateAttractionBookingStatus } from "@repo/actions";
import {
  ArrowRightCircle,
  Ban,
  CheckCircle,
...
type AttractionBookingStatus = "cancelled" | "closed" | "confirmed" | "contacted" | "pending";

type TAttractionBookingWithAttraction = TAttractionBooking & {
  attraction: { title: string } | null;
};

const STATUS_OPTIONS: { icon: React.ReactNode; label: string; value: AttractionBookingStatus }[] = [
...
interface CellActionProps {
  data: TAttractionBookingWithAttraction;
}
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onStatusChange = (status: AttractionBookingStatus) => {
    ...
    startTransition(async () => {
      try {
        await updateAttractionBookingStatus(data.id, status);
  ...
  const currentStatus = data.status as AttractionBookingStatus;
    ...
    <>
      ...
      <Dialog onOpenChange={setDetailsOpen} open={detailsOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Attraction Booking Details</DialogTitle>
            <DialogDescription>
              {data.attraction?.title || "Attraction booking"} request
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">
                  {new Date(data.travel_date + "T00:00:00").toLocaleDateString("en-US", {
                    day: "numeric",
                    month: "long",
                    year: "numeric",
                  })}
