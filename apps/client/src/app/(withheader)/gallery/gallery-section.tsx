...
export function GallerySection({ gallery }: GallerySectionProps) {
  ...
  return (
    <>
      {/* Category Tabs */}
      <section className="sticky top-0 z-30 border-b border-gray-200">
          ...
          <nav className="flex items-center justify-center gap-1 sm:gap-2 py-4 overflow-x-auto">
            {CATEGORIES.map((cat) => (
              <Button
                ...
                key={cat.value}
                onClick={() => setActiveCategory(cat.value)} variant={activeCategory === cat.value ? "default" : "outline"}>
                {cat.label}
              </Button>
        ...
        {filteredGallery.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 space-y-4">
            {filteredGallery.map((item, index) => {
              ...
              return (
                <div
                  className="group relative bg-background break-inside-avoid overflow-hidden rounded-lg cursor-pointer"
                  key={item.id}
